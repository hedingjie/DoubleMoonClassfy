...
class Perceptron(object):
    ...
    def train(self, data):
        ...
        while not learned:
            ...
            errorset.append(globalError)
            iteration += 1
            print globalError
            if globalError == 0.0 or iteration >= 100: # stop criteria
                print 'iterations: %s' % iteration
                learned = True # stop learing
        iters=[i for i in xrange(iteration)]
        plt.plot(iters,errorset,color="red")
        plt.xlabel("iteration")
        plt.ylabel("globalError")
        plt.title("iteration-globalError")                
        plt.savefig("iteration-globalError.png")
        #plt.show()